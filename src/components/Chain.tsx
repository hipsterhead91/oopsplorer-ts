...
function Chain(props: IChainProps) {
  ...
  const [totalBonded, setTotalBonded] = useState<string | null>(null); // получается один раз для расчёта voting power валидаторов и больше не меняется
  const [totalBondedUpdating, setTotalBondedUpdating] = useState<string | null>(null); // то же, что и totalBonded, но обновляется по таймеру
  const [unbondingTime, setUnbondingTime] = useState<number | null>(null);
  const [activeProposals, setActiveProposals] = useState<string | null>(null);
  const [blockHeight, setBlockHeight] = useState<string | null>(null);
  const [inflation, setInflation] = useState<string | null>(null);
  const [communityPool, setCommunityPool] = useState<string | null>(null);
  ...
  useEffect(() => {
    chainApi.getCommunityPool()
      .then(result => {
        const pool = result.pool.find((el: any) => el.denom === currentChain.denom);
